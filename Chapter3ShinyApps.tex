...
\km{ Note: This section has been moved up from "suvery" section of paper to more appropriate "applications" section \\ }
...
A second application of the Shiny Database Sampler was as a data source for a capstone project for two sections of the Stat 104 class, over the span of two semesters.  The project required students to define a research question that could be answered by investigating a bivariate relationship. The groups would then collect, visualize and analyze the bivariate data using what they had learned in the class. \\

While the format was a formal written report, the research question and data collection format were left open ended to allow students' interests to be incorporated.  Some groups chose to conduct experiments ranging from the dull (i.e. distance of ball roll vs angle of ramp) to the interesting (i.e. resistance genes in soy plants vs aphid population growth).  Other groups chose to run surveys with friends and discuss an observed bivariate relationship in the presence of sampling bias. A number of groups chose to run a mock survey, using the databases available through the Shiny Database Sampler as surrogates for large populations. 
