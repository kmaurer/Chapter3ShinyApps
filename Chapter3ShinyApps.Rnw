...
%\hh{the next paragraph is a bit lengthy without adding much to the content, let's comment it out for now.}
...
The Shiny Database Sampler allows the user to randomly sample subsets from remotely stored SQL databases using a point-and-click graphical user interface. The tool is available online through the link at \url{http://shiny.stat.iastate.edu/karstenm/ShinyDatabaseSampler/}.  A screenshot of the graphical user interface is shown in Figure 1 \hh{This Figure 1 is hardcoded. It would be good to highlight the main panel and side panel by putting frames in the appropriate places. I would suggest to use Figure 1 for that.}.  The interface is broken into two main sections: a main panel and a sidebar panel. The main panel is for displaying information whiel the side panel allows the user to pick from  a set of  options. The exact details of these sections depend on which tab of the application is selected. %; however these sections contain different options and displays depending on which tab of the application is selected.\\  
...
\hh{I would not feature the Recmiler data in such a prominent pole position. We do not know whether this data was collected with IRB approval. If not we will not be allowed to use it for publication.  }
...
the user decides between a simple or a stratified random subsample from the database. 
For the simple random sample the user only has to specify a sample size;
for a stratified random sample the strata variable and number of samples per stratum need to be specified.  
...
The remaining functionality of the sidebar allows the user to download the selected subsample (as comma separated value file)  to the default download folder on a local drive  on the user's computer.\\  
...
\km{  A second application of the Shiny Database Sampler was as a data source for a capstone project for three sections of the Stat 104 class.  The project required students to define a research question that could be answered by investigating a bivariate relationship. The groups would then collect, visualize and analyze the bivariate data using what they had learned in the class. \\  }

\km{  While the format was a formal written report, the research question and data collection format were left open ended to allow students' interests to be incorporated.  Some groups chose to conduct experiments ranging from the dull (i.e. distance of ball roll vs angle of ramp) to the interesting (i.e. resistance genes in soy plants vs aphid population growth).  Other groups chose to run surveys with friends and discuss an observed bivariate relationship in the presence of sampling bias. Six of groups chose to run a mock survey, using the databases available through the Shiny Database Sampler as surrogates for large populations.  An example of a project using the Shiny Database Sample is a group that looked at the association between the day of the week and number of drunk drivers involved in fatal car crashes through a sample from the Accidents database.  \\ }
